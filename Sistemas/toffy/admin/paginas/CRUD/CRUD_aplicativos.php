...
{ 
			...
			if(strcmp($_POST['CRUD_query'],"insert")==0)
			{
						...
						$nome = strtoupper(htmlentities($nome));
						$SQL = "INSERT INTO gl_aplicativos (nome,link) 
												VALUES ('$nome','$link');";
						...
						<div class="alert alert-success">
						    ...
						    <strong>Sucesso: </strong>Um novo aplicativo foi cadastrado! <br>
						</div>
						<br>
						<?
						}
			...
			if(strcmp($_POST['CRUD_query'],"update")==0)
			{
				...
				$link = mysql_real_escape_string($_POST['link']); 
						...
						<div class="alert alert-success">
							<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
						    <strong>Sucesso: </strong>Aplicativo alterado! <br>
						</div>
						<br>
						<?
						}
			...
			if(strcmp($_POST['CRUD_query'],"delete")==0)
			{
						...
						$codigo = $_POST['codigo'];					
						$SQL = "DELETE FROM gl_aplicativos WHERE codigo = '$codigo';";								
						$query = mysql_query($SQL);
						$erro_banco = mysql_errno($con)." : ".mysql_error($con);
						if(strcmp($erro_banco,"0 : ")==0)
						{
						...
						<div class="alert alert-success">
							<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
						    <strong>Sucesso: </strong> O Aplicativo foi excluído <br>
						</div>
						<br>
						<?
						}
			...
			if(!empty($_GET['id'])) 
			{
              if(strcmp($_GET['acao'],"excluir")!= 0)
			  {
				...
				if(strcmp($_GET['acao'],"alterar")==0)
				{
					...
					c.nome as nome,
					c.link as link
					FROM gl_aplicativos c WHERE c.codigo = '$id'
...
?>
		...
		<form id="form_cadastro" action="" method="post">
			<div class="row" >
				...
				<input type="text" class="form-control" id="link" name="link" placeholder="Insira um link para acessar o aplicativos" required value="<?= $link ?>" >
				</div>
			</div>
		<div class="row">
