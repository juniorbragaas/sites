...
<head>
  <link rel="stylesheet" type="text/css" href="<?= $caminho_servidor ?>/css/estilo.css"/>
	...
	<div id="cabecalho_pagina" width="100%" style="background-color: #827D7D; color: #FFFFFF;"><center><?= $titulo; ?></center></div>
		...
		<table style= "">
		<tr><td><label><b>Categoria</b><br></label>
...
?>
		...
		<td>
		<label><b>Empresa Solicitante</b><br></label>
...
?>
		...
		<tr>
		<td colspan="4"><b>Descrição do chamado</b></td>
	</tr>
	<tr>
		<td colspan="4"><textarea id="descricao" name="descricao" rows="8" cols="100"><?= $descricao ?></textarea></td>		
	</tr>
	<tr>
		<td><label><b>Prazo</b><br></label><input type="date" id="data_prazo" name="data_prazo" placeholder="Digite um nome" <?= $readonly ?> value="<?= $data_prazo ?>"></td>	
		</tr><tr>
		<td style="<?=$display_historico?>"><label><b>Responsável(código)</b><br></label><input type="text" value="<?=$codigo?>" <?= $readonly ?> ></td></tr>
		<tr><td colspan="2" style="<?= $display_encaminhar ?>"><label><b><?=$encaminhado?></b><br></label>
...
?>
				...
				?>
				<option value='0' <?=$selected_alterar?>>Não encaminhar</option>
		</select></td>
		<td></td>
	<tr>
<tr>
		<td colspan="5">Observação atual</td>
	</tr>
	<tr>
		<td colspan="5"><textarea id="<?=$nome_id?>" name="<?=$nome_id?>" rows="8" cols="100" <?= $readonlyobs ?>><?= $observacao_atual?>
</textarea></td>
		
	</tr>
	<tr>
		<td colspan="5" style="<?=$display_historico ?>">Observação</td>
	</tr>
	<tr style="<?=$display_historico ?>">
		<td colspan="5"><textarea id="observacao" name="observacao" rows="8" cols="100" >
</textarea></td>
</tr>
<tr style="<?=$display_historico ?>"><td><label>Status<br></label>
		<select id="status" name="status" style='width:400px;'  form="cadform">
		<option value='1' <? if(strcmp($status,'1') == 0) { echo 'Selected';} ?> >Pendente</option>
		<option value='2' <? if(strcmp($status,'2') == 0) { echo 'Selected';} ?> >Concluído</option>
		<option value='3' <? if(strcmp($status,'3') == 0) { echo 'Selected';} ?>>Cancelado</option>
</select></td></tr>		
		</table>
